...
const switchTab = (which) => {
  ...
  // Re-animate the cards when switching tabs
  nextTick(() => {
    if (stepsRef.value) {
      gsap.fromTo(stepsRef.value.querySelectorAll('.step-card'),
        { y: 30, opacity: 0 },
...
onMounted(async () => {
  ...
  ctx = gsap.context(() => {
    ...
    // Step cards stagger
    if (stepsRef.value) {
      tl.fromTo(stepsRef.value.querySelectorAll('.step-card'),
        { y: 25, opacity: 0 },
        { y: 0, opacity: 1, duration: 0.35, stagger: 0.04 },
        "-=0.2"
      )
    ...
    if (pipelineRef.value) {
      tl.fromTo(pipelineRef.value.querySelectorAll('.pipeline-card'),
        { y: 25, opacity: 0 },
        { y: 0, opacity: 1, duration: 0.35, stagger: 0.04 },
        "-=0.15"
      )
...
<template>
  <div class="bg-base pt-28">
    ...
    <section class="py-20 md:py-24 px-6 md:px-10 border-b border-border-color relative">
      ...
      <div class="absolute top-[30%] right-[20%] w-[400px] h-[400px] bg-brand-blue/[0.03] rounded-full blur-[120px]"></div>
      ...
      <div class="max-w-[1200px] mx-auto relative z-10">
        ...
        <!-- Tab Switcher -->
        <div ref="tabsRef" class="flex gap-2 mb-12 p-1.5 bg-surface-raised rounded-xl w-fit border border-border-color">
          <button @click="switchTab('project')" 
            :class="[
              ...
              activeTab === 'project' 
                ? 'bg-brand-blue text-white shadow-[0_0_20px_rgba(42,114,184,0.3)]' 
        ...
        <div ref="stepsRef" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          <div v-for="step in (activeTab === 'project' ? projectSteps : investorSteps)" :key="step.num + activeTab"
            ...
            <div class="flex items-center gap-3 mb-6">
              ...
              <span class="text-[11px] font-bold tracking-[0.15em] uppercase text-mist">Step {{ step.num }}</span>
            </div>
            <div class="text-xl font-bold text-ink mb-3 group-hover:text-brand-blue transition-colors">{{ step.title }}</div>
            <p class="text-[14px] text-mist leading-relaxed">{{ step.desc }}</p>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="py-20 md:py-24 px-6 md:px-10 relative">
      <div class="absolute inset-0 grid-pattern"></div>
      <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-brand-green/[0.03] rounded-full blur-[120px]"></div>

      <div class="max-w-[1200px] mx-auto relative z-10" ref="pipelineRef">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-12 mb-16 items-end">
          <div>
            <div class="section-label mb-6">
            ...
            <h2 class="text-[clamp(28px,3vw,44px)] font-extrabold leading-[1.1] tracking-tight text-ink">
              The <span class="gradient-text-gold">First Deal</span><br>
              Architecture Process
            </h2>
          </div>
          <p class="text-[15px] leading-relaxed text-mist">
            Every deal goes through our 5-stage structured process. We don't pass leads — we architect complete investment opportunities.
          </p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-3">
          <div v-for="(stage, idx) in pipeline" :key="idx"
            ...
            <div class="flex items-center gap-2 mb-6">
              ...
              <span class="text-[10px] font-bold tracking-[0.12em] text-brand-blue uppercase">Step 0{{ idx + 1 }}</span>
            </div>
            <div class="text-[15px] font-bold text-ink group-hover:text-brand-blue transition-colors leading-snug">{{ stage }}</div>
